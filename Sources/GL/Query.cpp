...
namespace GL
{
        ...
        m_Target( GL_NONE )
    {
        ...
        if (Error != GL_NO_ERROR)
            throw CException( this, Error, "::CQuery() : Failed to generate query object." );
    }
    ...
    // BeginQuery
    //
    void CQuery::BeginQuery( GLenum Target )
    {
    #ifdef _DEBUG
        switch (Target) 
        {
        ...
        default:
            throw Sys::CDeveloperException( this, Sys::CDeveloperException::ERR_INVALID_ENUM, 
    ...
    // IsCurrent
    //
    bool CQuery::IsCurrent() const
    {
        GLint CurrentQuery;

        glGetQueryiv( m_Target, GL_CURRENT_QUERY, &CurrentQuery );

        return m_Query == CurrentQuery;
    }

    //
    // IsResultAvailable
    //
    bool CQuery::IsResultAvailable() const
    {
    #ifdef _DEBUG
        glClearErrors();
    #endif
    ...
    // GetResult
    //
    void CQuery::GetResult( GLint *Result ) const
    {
    #ifdef _DEBUG
        glClearErrors();
    #endif
    ...
    // GetResult
    //
    void CQuery::GetResult( GLuint *Result ) const
    {
    #ifdef _DEBUG
        glClearErrors();
    #endif
    ...
    // GetResult
    //
    void CQuery::GetResult( GLint64EXT *Result ) const
    {
    #ifdef _DEBUG
        if (!m_Extensions->EXT_timer_query)
    ...
    // GetResult
    //
    void CQuery::GetResult( GLuint64EXT *Result ) const
    {
    #ifdef _DEBUG
        if (!m_Extensions->EXT_timer_query)
            throw CExtensionException( this, "::GetResult()", "GL_EXT_timer_query" );
