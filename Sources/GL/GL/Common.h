...
namespace GL
{
    ...
    DLLAPI void         glVertexAttrib2fv( int Index, const Vec2<GLfloat>& v );
    DLLAPI void         glVertexAttrib3fv( int Index, const Vec3<GLfloat>& v );
    DLLAPI void         glVertexAttrib4fv( int Index, const Vec4<GLfloat>& v );
    ...
    class DLLAPI CObject : public Sys::IRTTIObject, public Sys::CNonCopyable, public Sys::CLog
    {
    ...
    __M(
        m_Extensions,       static Ptr<const CExtensionRegistry>,
        m_InternalFormats[],static const CInternalFormat,
        m_Caps,             static TCaps,

        m_CgShaders,        static bool // Currently not used.
        );
    };
    ...
    //  Исключение, бросаемое при попытке использовать функциональность, 
    //  объявленную в версии OpenGL 3.0 и выше как deprecated.

    class DLLAPI CDeprecatedException : public Sys::CException
    {
    public:
                            CDeprecatedException( const Sys::IRTTIObject *Obj, const char *Method, int MajorNumber, int MinorNumber );
                            CDeprecatedException( const char *ClsName, const char *Method, int MajorNumber, int MinorNumber );
    };
} 
