...
namespace GL
{
    ...
    #ifdef CG_SDK
        m_APIType( IShadingProgram::SA_UNKNOWN ),
    #endif
    ...
    //
    // IsDeprecatedTopology
    //
    bool CRenderDevice::IsDeprecatedTopology( 
                                             GLenum Mode
                                             ) const
    {
        switch (Mode)
    ...
    // IsSupportedTopology
    //
    bool CRenderDevice::IsSupportedTopology( 
                                            GLenum Mode 
                                            ) const
    {
        switch (Mode)
        ...
        case GL_TRIANGLE_STRIP_ADJACENCY_EXT:
            return (m_Extensions->EXT_geometry_shader4 == true);

        default:
            throw Sys::CDeveloperException( this, "::IsSupportedTopology() : Invalid <Mode> parameter." );
        }
    ...
    // ToString
    //
    const char* CRenderDevice::ToString( 
                                        GLenum Mode 
                                        ) const
    {
        const char *ModeStr[] =
        ...
        return ModeStr[ Mode - GL_POINTS ];
    }

    //
    // IsVertexBufferBound
    //
    bool CRenderDevice::IsVertexBufferBound() const
    {
