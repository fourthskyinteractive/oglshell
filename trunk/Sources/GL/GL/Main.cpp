...
namespace GL
{
                               ...
                               )
    {
    ...
    #endif // _WIN32
        {
            throw Sys::CDeveloperException( Sys::CDeveloperException::ERROR_INVALID_CALL, 
        ...
        if (Extensions->ARB_map_buffer_range)
        {
        }

        CObject::Init( Extensions, false ); // ?

        if (Flags & CREATE_DEBUG_RENDER_DEVICE)
            *RenderDevice = new GL::CRenderDeviceDebug();
        else
            *RenderDevice = new GL::CRenderDeviceRelease();
