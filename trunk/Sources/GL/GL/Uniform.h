...
namespace GL
{
    ...
    public:
        ...
        virtual void        SetUnsignedIntArray ( GLsizei Dimension, GLsizei Count, const GLuint *Array );  // GL_EXT_gpu_shader4

    #ifdef MATH_LIB_INCLUDED
        virtual void        SetFloatVec2        ( const Vec2<GLfloat>& v );
        virtual void        SetIntVec2          ( const Vec2<GLint>& v );
        virtual void        SetUnsignedIntVec2  ( const Vec2<GLuint>& v );                                  // GL_EXT_gpu_shader4
        virtual void        SetFloatVec3        ( const Vec3<GLfloat>& v );
        virtual void        SetIntVec3          ( const Vec3<GLint>& v );
        virtual void        SetUnsignedIntVec3  ( const Vec3<GLuint>& v );                                  // GL_EXT_gpu_shader4
        virtual void        SetFloatVec4        ( const Vec4<GLfloat>& v );
        virtual void        SetIntVec4          ( const Vec4<GLint>& v );
        virtual void        SetUnsignedIntVec4  ( const Vec4<GLuint>& v );                                  // GL_EXT_gpu_shader4

        virtual void        SetFloatMat2        ( const Mat2& m, EMatrixOrder Order );
        virtual void        SetFloatMat3        ( const Mat3& m, EMatrixOrder Order );
        virtual void        SetFloatMat4        ( const Mat4& m, EMatrixOrder Order );
    #endif // MATH_LIB_INCLUDED

    private:
    #ifdef _DEBUG
        void                Try                 ( const char *Name, GLenum Type ) const;
        void                Catch               ( const char *Name ) const;
    #endif
